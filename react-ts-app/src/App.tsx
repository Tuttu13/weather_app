...
const WeatherApp: React.FC = () => {
    ...
    useEffect(() => {
        if (city) {
            axios.get<WeatherData>(`${process.env.REACT_APP_OW_API_URL}?q=${city}&lang=${process.env.REACT_APP_OW_API_LANG}&appid=${process.env.REACT_APP_OW_API_KEY}`)
                    ...
                    <TableBody>
                      {weatherData && (
                        <TableRow>
                          ...
                          <TableCell>{(weatherData.main.temp - 273.15).toFixed(1)} °C</TableCell> {/* ケルビンから摂氏に変換 */}
